...
- **다크모드 토글** — 웹과 같은 규칙을 그대로 가져온다. 설정 창이 아니라
  팝오버에 있다 (4.3)
- **시계 페이스 10종** — "팝오버가 작아 차이가 드러나지 않는다"가 제외의
  근거였는데, 그렇다면 팝오버를 키우면 될 일이었다. 페이스를 전부 옮기고
...
│    ╱ 12  ╲      │   고른 페이스 10종 중 하나 — 웹 SVG를 수치 그대로 옮김
│   9   ┼   3    │
│    ╲  6  ╱      │
│                 │
...
채운다. 5.3의 비례 축소가 주는 132pt보다 크게 잡은 이유는 페이스가 열 종으로
늘어났기 때문이다: 숫자판·남은·해시계처럼 글자와 잔눈금이 들어가는 페이스는
132pt에서 읽히지 않아, 고를 수 있게 만든 의미가 사라진다. 글꼴 크기가 아니라
도형 크기라 5.3의 규칙과 충돌하지도 않는다.

페이스는 자기 지름을 모른다 — 전부 `viewBox="0 0 200 200"` 좌표계로 그리고
...
| 항목 | 비고 |
|---|---|
| 시계 | 페이스 10종을 가로로 늘어놓고 실제로 그려서 고른다. 웹과 같이 급여보다
...
**누르는 것은 전부 칸 전체가 과녁이다.** SwiftUI는 버튼 라벨에 `frame`이나
배경을 줘도 히트 영역을 실제로 그려진 내용만큼만 잡는다. 그래서 투명한 배경을
가진 칸은 글자나 그림에 정확히 닿아야만 눌리고, 배경이 칠해진 칸은 아무 데나
눌린다 — 같은 화면에서 고르기 전과 후의 반응이 달라진다. `contentShape`로
칸 전체를 히트 영역으로 넓힌다. 시계 페이스 고르는 칸, 그 좌우 화살표,
팝오버의 아이콘 버튼, 달력 날짜 칸이 모두 해당한다.

달력의 날짜 칸이 그중 가장 심했다. SwiftUI에서 `Text`에 `frame`으로
...
공휴일 표가 없는 해(2031~)에는 주말만 근무일수 계산에 반영하고,
웹과 같은 문구로 그 사실을 알린다. 달력을 펼쳤는지·어느 달을
보여주는지는 `Settings`에 저장하지 않는 순수 뷰 상태다 — 웹도 같다.

달력 헤더의 화살표로 달을 옮긴다. 창을 열 때는 늘 이번 달에서 시작한다.
다음 달 연차를 미리 찍어 두려면 이동이 필요하고, 찍은 날짜(`dayOverrides`)는
절대 날짜라 그 달이 왔을 때 근무일수에 반영된다. 설정 화면 위쪽의 근무일수
숫자는 이번 달 기준 그대로이고, 달력 헤더가 보여주는 근무일수는 지금 보고 있는
달의 것이다.
...
`rose`·`sky` 몇 토큰을, 시계 페이스 10종(4.3)이 `stone`·`amber`·`orange`와
...
| 시계 페이스 10종 | 위 역할 이름으로 묶이지 않는 색은 `Theme.pair(밝게, 어둡게)`로
...
| 공휴일 표가 없는 해 (2031~) | 주말만 휴무로 보고, 설정 창에 그 사실을 표시 |
...
말끔 페이스를 그대로 키운 그림이다 — 바깥에 진행 링, 안에 눈금과 바늘. 메뉴바
...
원래 여기 있던 "달력 근무일 지정"·"다크모드 고정 토글"과 "시계 페이스
